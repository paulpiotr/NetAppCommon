...
namespace NetAppCommon.AppSettings.Models.Base
{
    ...
    public class AppSettingsBaseModel : RsaProvaiderBaseModel
    {
        ...
        public virtual AppSettingsRepositoryBase<AppSettingsBaseModel> AppSettingsRepository
        {
            get
            {
                if (null == _appSettingsRepository)
                {
        ...
        public AesIVProviderService AesIVProviderService
        {
            get
            {
                if (null == _aesIVProviderService)
                {
                    _aesIVProviderService = new AesIVProviderService();
                }
                return _aesIVProviderService;
            }
            set
            {
        ...
        protected RsaProviderService _rsaProviderService;// = (RsaProviderService)MemoryCacheProvider.GetInstance().Get(string.Format("{0}{1}", MethodBase.GetCurrentMethod().DeclaringType.Name, "RsaProviderService"));
        ...
        public RsaProviderService RsaProviderService
        {
            get
            {
                ...
                {
                    //Console.WriteLine($"RsaProviderService RsaProviderService { null == AsymmetricPrivateKeyFilePath || !File.Exists(AsymmetricPrivateKeyFilePath) || null == AsymmetricPublicKeyFilePath || !File.Exists(AsymmetricPublicKeyFilePath) }");
                    //_rsaProviderService = RsaProviderService.GetRsaProviderService(
                    _rsaProviderService = new RsaProviderService(
                        AsymmetricPrivateKeyFilePath,
                        AsymmetricPublicKeyFilePath,
                        null == AsymmetricPrivateKeyFilePath || !File.Exists(AsymmetricPrivateKeyFilePath) || null == AsymmetricPublicKeyFilePath || !File.Exists(AsymmetricPublicKeyFilePath)
                        );
                    _rsaProviderService.SaveAsymmetricKeyPairToFile(AsymmetricPrivateKeyFilePath, AsymmetricPublicKeyFilePath, true);
                    //MemoryCacheProvider.GetInstance().Put(string.Format("{0}{1}", MethodBase.GetCurrentMethod().DeclaringType.Name, "RsaProviderService"), _rsaProviderService, TimeSpan.FromDays(1));
                }
                return _rsaProviderService = (RsaProviderService)(MemoryCacheProvider.GetInstance().Get(string.Format("{0}{1}", MethodBase.GetCurrentMethod().DeclaringType.Name, "RsaProviderService")) ?? _rsaProviderService);
            }
            set
            {
            ...
            get
            {
                try
                {
                    if (null == _userProfileDirectory)
                    {
                        ...
                        {
                            _userProfileDirectory = Path.Combine(GetFolderPath(SpecialFolder.UserProfile), Assembly.GetExecutingAssembly().GetName().Name);
                            if (!Directory.Exists(_userProfileDirectory))
                            {
        ...
        public virtual string FilePath
        {
            get
            {
                if (null == _filePath)
                {
                    if (null != BaseDirectory && null != FileName)
                    {
        ...
        public virtual IConfigurationBuilder AppSettingsConfigurationBuilder
        {
            get
            {
                if (null == _appSettingsConfigurationBuilder && null != FilePath && !string.IsNullOrWhiteSpace(FilePath) && File.Exists(FilePath))
                {
                    _appSettingsConfigurationBuilder = new ConfigurationBuilder().SetBasePath(Path.GetDirectoryName(FilePath)).AddJsonFile(Path.GetFileName(FilePath), optional: true, reloadOnChange: true);
                }
        ...
        public virtual IConfigurationRoot AppSettingsConfigurationRoot
        {
            get
            {
                if (null == _appSettingsConfigurationRoot && null != FilePath && !string.IsNullOrWhiteSpace(FilePath) && File.Exists(FilePath))
                {
        ...
        [JsonIgnore]
        [XmlIgnore]
        [Display(Name = "Wymuś instalację i aktualizację bazy danych", Prompt = "Zaznacz dla wykonania wymuś instalację i aktualizację bazy danych", Description = "Wymuś instalację i aktualizację bazy danych")]
        ...
        [Display(Name = "Okres istnienia pamięci podręcznej (w sekundach)", Prompt = "Wpisz okres istnienia pamięci podręcznej (w sekundach)", Description = "Okres istnienia pamięci podręcznej (w sekundach)")]
        [Range(0, 2147483647)]
        [Required]
        ...
        /// </summary>
        [Display(Name = "Używaj globalnych ustawień połączenia bazy danych z pliku ustawień appsettings.json", Prompt = "Zaznacz, jeśli chcesz używać globalnych ustawień połączenia bazy danych z pliku ustawień appsettings.json", Description = "Używaj globalnych ustawień połączenia bazy danych z pliku ustawień appsettings.json")]
        public bool UseGlobalDatabaseConnectionSettings
        {
            get
            {
                try
                {
                    if (null == _useGlobalDatabaseConnectionSettings)
                    {
                        _useGlobalDatabaseConnectionSettings = AppSettingsRepository.GetValue<bool>(this, "UseGlobalDatabaseConnectionSettings");
                    }
                }
                catch (Exception e)
                {
                ...
                return (bool)_useGlobalDatabaseConnectionSettings;
            }
            set
            {
        ...
        #region protected bool _cheskForConnection; public bool CheskForConnection

        protected bool _cheskForConnection;
        ...
        public virtual string ConnectionString
        {
            get
            {
                if (null == _connectionString)
                {
                    try
                    {
                        _connectionString = AppSettingsRepository.GetValue<string>(this, string.Format("{0}:{1}", nameof(ConnectionStrings), ConnectionStringName));
                        if (string.IsNullOrWhiteSpace(_connectionString))
                        {
                            _connectionString = $@"Data Source=(LocalDB)\MSSQLLocalDB; AttachDbFilename=%Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)%\MSSQLLocalDB\MSSQLLocalDB.mdf; Database=%AttachDbFilename%; MultipleActiveResultSets=true; Integrated Security=SSPI; Trusted_Connection=Yes; Max Pool Size=65536; Pooling=True";
                        }
                    }
                    catch (Exception e)
                    {
                    ...
                    finally
                    {
                        if (null != _connectionString && _connectionString.Length <= 512)
                        {
                            ...
                            _connectionString = AesIVProviderService.Encrypt(_connectionString, RsaProviderService.AsymmetricPublicKeyAsString);
                        }
                    }
                }
            ...
            set
            {
                if (value != _connectionString)
                {
                    ...
                    if (null != ConnectionStringName && !string.IsNullOrWhiteSpace(ConnectionStringName) && null != _connectionString && !string.IsNullOrWhiteSpace(_connectionString))
                    {
        ...
        public virtual string GetConnectionString()
        {
            ...
            try
            {
                ...
                {
                    connectionString = AesIVProviderService.Decpypt(ConnectionString, RsaProviderService.AsymmetricPublicKeyAsString);
        ...
        public virtual DbContextOptionsBuilder DbContextOptionsBuilder
        {
            ...
            set
            {
                if (value != _dbContextOptionsBuilder)
                {
        ...
        /// <summary>
        /// public virtual DbContextOptionsBuilder GetDbContextOptionsBuilder<TContext>()
        /// public virtual DbContextOptionsBuilder GetDbContextOptionsBuilder<TContext>()
        /// </summary>
        ...
        public virtual DbContextOptionsBuilder GetDbContextOptionsBuilder<TContext>() where TContext : DbContext
        {
            try
            {
                if (null == DbContextOptionsBuilder && null != ConnectionString)
                {
        ...
        protected DbContextOptions _dbContextOptions = default;
